...
### Changed
- CI now also runs the test suite on Ruby 4.0 ([#113](https://github.com/Coolhand-Labs/coolhand-ruby/pull/113)). The development Gemfile now uses `debug` on every Ruby (instead of swapping `pry-byebug` for `debug` on 4.0+ at resolve time) and `Gemfile.lock` no longer pins `BUNDLED WITH`; the old conditional Gemfile and the `2.3.6` pin together made frozen `bundle install` fail on the new Ruby 4.0 job.
