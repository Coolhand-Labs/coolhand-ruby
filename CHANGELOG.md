...
- **An opt-in live suite (`bundle exec rake spec:live`)** — drives the template methods against a real Coolhand server with no stubbing, from `COOLHAND_LIVE_BASE_URL`/`COOLHAND_LIVE_API_KEY`, hard-failing if either is missing. It lives in `spec/live/*_live.rb`, which RSpec's default pattern — and so `rake` and CI — never matches. Every request it makes is a read.
